...
def river_loop():
    ...
    win.blit(DECK[1].image,(gap+(card_width * 8), int(display_height*0.3)))
    pygame.display.update()    
...
def turn_loop():
    ...
    win.blit(DECK[0].image,(gap+(card_width * 7), int(display_height*0.3)))
    pygame.display.update()
...
def flop_loop():
    ...
    win.blit(Flop_Dealt.card1.image,(gap+(card_width * 4), int(display_height*0.3)))
    win.blit(Flop_Dealt.card2.image,(gap+(card_width * 5), int(display_height*0.3)))
    win.blit(Flop_Dealt.card3.image,(gap+(card_width * 6), int(display_height*0.3)))
    pygame.display.update()
...
Hand_1, DECK = pf.Deal(Deck_Shuffled,Players)

Starting_Chips = 3000 #int(input("Starting Chips: "))
Chips=[]
for index in range(Players):
    Chips.append(Starting_Chips)
for index in range(len(Hand_1)):
    print(Hand_1[index])

"""Deal community cards"""

Flop_Dealt, DECK = pf.Deal_Flop(DECK)
Turn = DECK[0]
River = DECK[1]
...
for i in range(Players):
    ...
    globals()['player'+str(i)].dealt_hand = Hand_1[i]
    if i == Players - 1:
