...
print('-----')
##print(inspect.signature(itertools.combinations))
##print(dir(max))
##print(max.__doc__)

##for param in inspect.signature(max).parameters.values():
##    print(param.name)
...
def best_hand(hand,cards_2):
    ...
    return max(combinations(cards, 5), key=hand_score)

print(Card._fields)
##cards = parse_cards(['Ah', '8h', '9h', '3h', 'Jh', 'Qc', 'Ac'])
##cards_2 = parse_cards(['Ah', '8h', '9h', '3h', 'Jh', 'Qc', 'Ac'])
table = parse_cards(['3s', '5s', '2s', 'Ks', 'Ah'])
hand = parse_cards(['4c', '6s'])
cards = hand + table
